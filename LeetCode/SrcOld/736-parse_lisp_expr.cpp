...
int main()
{
	OutExpr(evaluate("(add 1 2)"), "%d");
	OutExpr(evaluate("(mult 3 (add 2 3))"), "%d");
	OutExpr(evaluate("(let x 2 (mult x 5))"), "%d");
	OutExpr(evaluate("(let x 2 (mult x (let x 3 y 4 (add x y))))"), "%d");
	OutExpr(evaluate("(let x 3 x 2 x)"), "%d");
	OutExpr(evaluate("(let x 1 y 2 x (add x y) (add x y))"), "%d");
	OutExpr(evaluate("(let x 2 (add (let x 3 (let x 4 x)) x))"), "%d");
	OutExpr(evaluate("(let a1 3 b2 (add a1 1) b2)"), "%d");
}
