...
开始时，青蛙默认已站在第一个石子上，并可以假定它第一步只能跳跃一个单位（即只能从单元格 1 跳至单元格2）。
...
跳 1 个单位到第2块石子, 然后跳2个单位到第3块石子, 接着
跳 2 个单位到第4块石子, 然后跳3个单位到第6块石子,
跳 4 个单位到第7块石子, 最后，跳5个单位到第8个石子（即最后一块石子）。
...
class Solution
{
	...
	bool cross(int cur, int prestep)
	{
		if (prestep <= 0)
			return false;
		...
		int left = cur + prestep - 1;
		if (left <= dst && dst <= left + 2)
			return true;
		return
			cross(left + 2, prestep + 1) ||
			cross(left + 1, prestep + 0) ||
			cross(left + 0, prestep - 1);
	}

public:
	bool canCross(vector<int>& stones)
	{
		int const len = static_cast<int>(stones.size());
		if (stones[1] != 1)
			return false;
		dst = stones.back();
		pos.clear();
...
int main()
{
	...
	OutExpr(boolstr[Solution().canCross(stones)], "%s");
};
