...
/* 002 求两个单链表的和
两个单链表 (singly linked list)，每一个节点里面一个 0-9 的数字， 输入就相当于两个大数了。
...
// 将数组转换成链表
Node* mkList(char const* arr, int n)
{
...
int main()
{
	// printf("sizeof(Node): %zd", sizeof(Node)); // 16
	int const n = 10;
	char arr[n] = { 4, 9, 5, 3, 5, 6, 5, 6, 8, 1 };
	char brr[n] = { 5, 3, 0, 9, 7, 3, 9, 6, 4, 1 };
	int carry;
