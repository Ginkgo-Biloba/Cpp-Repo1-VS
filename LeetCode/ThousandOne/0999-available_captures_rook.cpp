...
{
	unsigned rows = static_cast<unsigned>(board.size());
	unsigned cols = static_cast<unsigned>(board[0].size());
	x += dx;
	y += dy;
	while (static_cast<unsigned>(x) < rows
		&& static_cast<unsigned>(y) < cols)
	{
		if (board[y][x] == 'B')
			return 0;
		if (board[y][x] == 'p')
